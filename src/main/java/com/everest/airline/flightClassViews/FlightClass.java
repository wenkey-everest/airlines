...
public abstract class FlightClass {
  ...
  public double priceByDate(LocalDate departureDate, int availableSeats, int classCapacity, double basePrice){
      ...
      if(beforeDate>15)
          ...
          return classFare + (classFare * validateDate(departureDate,10,3,0.02));
      }

      else
          return classFare + (classFare * validateDate(departureDate,3,0,0.1));
  }
  public double validateDate(LocalDate departureDate, int dateOneValue, int dateTwoValue, double incrementerValue){
